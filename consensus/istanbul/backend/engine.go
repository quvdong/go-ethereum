...
var (
	...
	// errInvalidCommittedSeals is returned if the field of committed seals is not appeared in parent validators.
...
// verifyCommittedSeals checks whether the committed seal is signed by parent's validator
func (sb *simpleBackend) verifyCommittedSeals(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	// The length of Committed seals should lager than 0
	if len(extra.CommittedSeal) == 0 {
		return errEmptyCommittedSeals
	}

	validators := snap.validators()
	// Checks whether the committed seals are generated by parent validators
	validSeal := 0
	// 1. Getting committed seals from current header
	for _, seal := range extra.CommittedSeal {
		// 2. gets the original address by using seal and parent block hash
		...
		found := false
		// 3. checks whether the address is appeared in parent validators
		for _, parentValidator := range validators {
			if parentValidator == addr {
				found = true
				validSeal += 1
				break
			}
		...
		// 4. returns error if seal is not signed by parent validator.
		if !found {
			return errInvalidCommittedSeals
		}
	}

	// The length of validSeal should large than number of faulty node + 1
