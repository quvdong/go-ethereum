...
func (pbft *pbft) VerifyHeaders(chain consensus.ChainReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
	...
	go func() {
		for i := range headers {
			results <- pbft.VerifyHeader(chain, headers[i], seals[i])
		}
	}()
...
func (pbft *pbft) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
	return nil
}
...
func (pbft *pbft) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction,
...
// seal place on top.
func (pbft *pbft) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error) {
	return block, nil
}
