...
func CreateConsensusEngine(ctx *node.ServiceContext, config *Config, chainConfig *params.ChainConfig, db ethdb.Database) consensus.Engine {
	...
	powConfig := config.Ethash
	switch {
	case powConfig.PowMode == ethash.ModeFake:
		log.Warn("Ethash used in fake mode")
		return ethash.NewFaker()
	case powConfig.PowMode == ethash.ModeTest:
		log.Warn("Ethash used in test mode")
		return ethash.NewTester()
	case powConfig.PowMode == ethash.ModeShared:
		log.Warn("Ethash used in shared mode")
		return ethash.NewShared()
	default:
		engine := ethash.New(ethash.Config{
			CacheDir:       ctx.ResolvePath(powConfig.CacheDir),
			CachesInMem:    powConfig.CachesInMem,
			CachesOnDisk:   powConfig.CachesOnDisk,
			DatasetDir:     powConfig.DatasetDir,
			DatasetsInMem:  powConfig.DatasetsInMem,
			DatasetsOnDisk: powConfig.DatasetsOnDisk,
		})
...
// DetermineEtherbase returns the etherbase for an Ethereum service
