...
func (pm *PBFTProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == NodeVote && p.version >= pbft101:
		var message []byte
		if err := msg.Decode(&message); err != nil {
			return errResp(ErrDecode, "msg %v: %v", msg, err)
		}
		log.Info("Received a vote", "from", string(message))
...
func (pm *ProtocolManager) BroadcastVote() {
	...
	for _, peer := range peers {
		peer.SendNodeVote([]byte(fmt.Sprintf("%s vote is true", peer.id)))
	}
	log.Info("Broadcast a vote", "receipts", len(peers))
}
